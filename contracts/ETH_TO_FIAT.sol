...
contract ETH_TO_FIAT {
    ...
    modifier onlySender(uint256 _roomNumber) {
        require(msg.sender == roomNumberMap[_roomNumber].sender, "Not sender");
        _;
    }
    ...
    function approveFromSender(uint256 _roomNumber, uint256 _counter) external onlySender(_roomNumber) {
    ...
    function stopRoom(uint256 _roomNumber) external onlySender(_roomNumber) {
    ...
    function continueRoom(uint256 _roomNumber) external onlySender(_roomNumber) {
        roomNumberMap[_roomNumber].roomStatus = RoomStatus.Continue;
    }
    ...
    function scamFromSender(uint256 _roomNumber, uint256 _counter) external onlySender(_roomNumber){
    ...
    function getRoomStatic(uint256 _roomNumber) public view returns(
        address, uint256, uint64, uint256, uint256, uint256, uint256
        ) {
        return(
    ...
    function getRoomDynamic(uint256 _roomNumber) public view returns(
        bool, bool, bool, AdminDecision, uint256, uint256, RoomStatus
    ) {
        return(
